...
static void my_application_activate(GApplication *application) {
  ...
  const int depth = depth_str ? atoi(depth_str) : 0;
  const bool use_custom_frame = depth == 0;
  bdw->setCustomFrame(use_custom_frame);
  ...
  gdk_rgba_parse(&background_color, "#0D0D0D");
...
MyApplication *my_application_new(int argc, char **argv) {
  ...
  char appId[128];
  if (depth == 0) {
    sprintf(appId, "com.twilightevd.vidra");
  } else {
    if (name) {
      sprintf(appId, "com.twilightevd.vidra.n.%s", name);
      flags = (GApplicationFlags)(G_APPLICATION_HANDLES_COMMAND_LINE |
                                   G_APPLICATION_SEND_ENVIRONMENT);
    } else {
      sprintf(appId, "com.twilightevd.vidra.p%d", getpid());
